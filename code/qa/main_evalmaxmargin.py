...
class Model(object):
    ...
    def evaluate(self, data, sess):
        ...
        def eval_batch(titles, bodies, labels, max_margin_loss):
            _scores = sess.run(
                self.scores,
                feed_dict={
            ...
            mml = self.max_margin_loss(labels, _scores)
            if mml is not None:
                max_margin_loss = (max_margin_loss + mml) / 2.
            return _scores, max_margin_loss

        for idts, idbs, id_labels in data:
            cur_scores, hinge_loss = eval_batch(idts, idbs, id_labels, hinge_loss)
    ...
    def train_model(self, train_batches, dev=None, test=None, assign_ops=None):
        with tf.Session() as sess:
            ...
            dev_summary_dir = os.path.join(args.save_dir, "summaries", "dev")
