...
class Model(object):
    ...
    def __init__(self, args, embedding_layer):
        self.args = args
        if args.concat:
            assert self.args.hidden_dim % 2 == 0, ' can not concat. either use concat 0 or an even hidden dimension '
    ...
    def _initialize_graph(self):
        ...
        with tf.name_scope('LSTM'):
            ...
            forward_cell = lstm_cell(self.args.hidden_dim/2 if args.concat else self.args.hidden_dim)
            backward_cell = lstm_cell(self.args.hidden_dim/2 if args.concat else self.args.hidden_dim)
