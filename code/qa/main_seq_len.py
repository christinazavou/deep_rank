...
class Model(object):
    ...
    def _initialize_graph(self):
        ...
        with tf.name_scope('titles_output'):
            ...
            self.t_states_series_cp, self.t_current_state_cp = tf.nn.dynamic_rnn(
                cell,
                self.titles,
                dtype=tf.float32
            )
            ...
            mask = tf.not_equal(self.titles_words_ids_placeholder, self.padding_id)
        ...
        with tf.name_scope('bodies_output'):
            ...
            self.b_states_series_cp, self.b_current_state_cp = tf.nn.dynamic_rnn(
                cell,
                self.bodies,
                dtype=tf.float32
            )
        ...
        with tf.Session() as sess:
            ...
            for epoch in xrange(max_epoch):
                ...
                def train_batch(titles, bodies, pairs):
                    ...
                    tstateseries, tcurrentstate, slt, tstateseriescp, tcurrentstatecp, mask = sess.run(
                        [self.t_states_series, self.t_current_state, self.SLT, self.t_states_series_cp, self.t_current_state_cp, self.mask],
                        feed_dict={
                            self.titles_words_ids_placeholder: titles.T,  # IT IS TRANSPOSE ;)
                            self.bodies_words_ids_placeholder: bodies.T,  # IT IS TRANSPOSE ;)
                            self.pairs_ids_placeholder: pairs,
                    ...
                    print 'mask ', mask.shape  # (1412, 27, 1)
                    # print 'tstateseries ', tstateseries.shape  # (1412, 27, 200)
                    print 'tcurrentstate ', tcurrentstate
                    # print 'slt ', slt.shape  # (1412,)
                    # print 'tstateseriescp ', tstateseriescp.shape  # (1412, 27, 200)
                    print 'tcurrentstatecp ', tcurrentstatecp
                    # print 'idx ', slt[0]
                    # idx = min(100, slt[0]+3)
                    # print '1 ', tstateseries[0, slt[0]:idx, 0:10]
                    # print '2 ', tstateseriescp[0, slt[0]:idx, 0:10]
                    # print 'idx ', slt[1]
                    # idx = min(100, slt[1]+3)
                    # print '1 ', tstateseries[1, slt[1]:idx, 0:10]
                    # print '2 ', tstateseriescp[1, slt[1]:idx, 0:10]
                    print 'last ', tstateseries[10, slt[10]-1, 0:10]
                    print 'last ', tstateseries[10, slt[10], 0:10]
                    print 'last ', tcurrentstate[0][1][10, 0:10]
                    print 'mask ', mask[10, :, :]
                    print 'idx ', slt[10]

                for i in xrange(N):
                    idts, idbs, idps = train_batches[i]
                    print 'idts idbs idps ', idts.shape, idbs.shape, idps.shape
                    train_batch(idts, idbs, idps)
...
def main(args):
    ...
    if args.dev:
        ...
        # todo: here if you had right pad it means you better take the average hidden state otherwise if padding is
        # left you can take the last hidden state
        # however in my case both will work ..
        # todo: but if i want to use sequence_length it means the padding should be on the right
        # so for the moment i skip it
    if args.test:
